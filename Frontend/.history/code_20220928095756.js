...
if (!window.location.href.includes("index.html")){
    ...
    var profilePictures = document.querySelectorAll("#profilePic");
    ...
    for (let y = 0; y < profilePictures.length; y++){
        console.log("usernames")
        profilePictures[y].addEventListener("click", function(){
            console.log(usernames[y]);
            if (usernames[y].innerHTML == window.localStorage.getItem("username")){
                window.location.href = "profile.html";
                personalProfile();
            } 
            else{   // other person page
                window.location.href = "profile.html";
            }
        })
    }
