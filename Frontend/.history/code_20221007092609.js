    ...
    for (let x = 0; x < data.length; x++){
        ...
        makeComment.addEventListener("click", function(){ //////// sets for every comment // get out of loop // should really change
            ...
            if (commentmade != ""){
                ...
                fetch(`http://localhost:8000/api/comments/create/${postarray[x]}/${userid}`, { 
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
if (window.location.href = "post.html"){
    var getcommentsfrompostid = window.localStorage.getItem("postid");
    getcomments(getcommentsfrompostid);
}

function getcomments(postid){
    fetch(`http://localhost:8000/api/comments/${postid}/${commentpagenumber}/${userid}`)
    .then(response => response.json())
    .then(data => {
