...
function getnotifications(){
    notifbell = document.querySelector(".notif");
    var userid = window.localStorage.getItem("userid");
    console.log(userid)
    ...
    .then(data => {
        ...
        notifbell.addEventListener("click", ()=>{
            ...
            notifdropDown.innerHTML = `${data[0].username} has requested to be a moderator in ${data[0].community}`;
        })
    });
}
...
function populatePosts(data, pagenumber){
    ...
    for (let x = 0; x < data.length; x++){
        ...
        commentbtnEl.innerHTML = `Tails: ${data[x].commentsnumber}`;
